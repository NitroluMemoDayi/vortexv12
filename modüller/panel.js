...
//Tovbe tovbe kufurlere bak 
 ...
 const renderTemplate = (res, req, template, data = {}) => {
  ...
  app.get("/callback", passport.authenticate("discord", { failureRedirect: "/autherror" }), async (req, res) => {
    ...
    client.channels.cache.get("632476440897650698").send(`**${client.users.get(req.user.id).tag}** adlı kullanıcı Web Paneline Discord hesabıyla giriş yaptı!`)
...
if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
  ...
  app.get("/panel/:sunucuID/guvenlik3/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`guvenlik3_${req.params.sunucuID}`) === false) return res.json({"hata": "Guvenlik kanalı "+client.guilds.cache.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
...
if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
  ...
  app.get("/panel/:sunucuID/komutkanal/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`ktr_${req.params.sunucuID}`) === false) return res.json({"hata": "Komut kanalı "+client.guilds.cache.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
...
if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
  ...
  app.get("/panel/:sunucuID/gresim/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`gcc_${req.params.sunucuID}`) === false) return res.json({"hata": "Resim kanalı"+client.guilds.cache.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
...
if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
  ...
  app.get("/panel/:sunucuID/destekk/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`destekK_${req.params.sunucuID}`) === false) return res.json({"hata": "Destek kanalı "+client.guilds.cache.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
   ...
   app.get("/panel/:sunucuID/destekr/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`destekR_${req.params.sunucuID}`) === false) return res.json({"hata": "Destek Rolü "+client.guilds.cache.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
...
// GENEL AYARLAR
  ...
  if (!isManaged && !req.session.isAdmin) return res.json({"hata":"Bu sunucuda Sunucuyu Yönet iznin bulunmuyor. Bu yüzden bu sayfaya erişim sağlayamazsın."});
   ...
   app.get("/panel/:sunucuID/ktr/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`ktr_${req.params.sunucuID}`) === false) return res.json({"hata": "Guvenlik kanalı "+client.guilds.cache.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
    client.ayar.delete(`ktr_${req.params.sunucuID}`)
...
if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
  ...
  app.get("/panel/:sunucuID/srol/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`sRol_${req.params.sunucuID}`) === false) return res.json({"hata": "Susturma rolü "+client.guilds.cache.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
