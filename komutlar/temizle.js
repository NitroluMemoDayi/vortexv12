...
exports.run = async function(client, message, args) {
 ...
 //if (!message.member.hasPermission("MANAGE_MESSAGES")) return message.reply(`Bu komutu kullanabilmek için **Mesajları Yönet** iznine sahip olmalısın!`);
  ...
  let fetched = await message.channel.message.fetchs({limit: args[0]})
  
  message.channel.bulkDelete(fetched)
  .catch(error => message.channel.send(`${dil.temizle.hata}`))
   message.channel.message.fetchs({limit: args[0]}).then(mesajlar => {

 let silindi = mesajlar.array().length;
    message.channel.bulkDelete(mesajlar);
  message.channel.send(`${client.emojis.cache.get(client.emojiler.konfeti)} \`${silindi}\` ${dil.temizle.silindi}`).then(msg => 	msg.delete(3000))
