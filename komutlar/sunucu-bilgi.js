...
let kur = {
	...
	const embed = new Discord.MessageEmbed()
		...
		.addField(`Kanallar [${message.guild.channels.size}]`, `${message.guild.channels.cache.filter(c => c.type === "text").size} Yazı  \n${message.guild.channels.cache.filter(c => c.type === "voice").size} Ses \n${message.guild.channels.cache.filter(c => c.type === 'category').size} Kategori`)
		.addField(`Kullanıcılar [${message.guild.members.size}]`, `${client.emojis.cache.get(client.emojiler.çevrimiçi)} ${message.guild.members.cache.filter(m => m.user.presence.status === "online").size} Çevrimiçi \n${client.emojis.cache.get(client.emojiler.boşta)} ${message.guild.members.cache.filter(m => m.user.presence.status === "idle").size} Boşta \n${client.emojis.cache.get(client.emojiler.yayıncı)} ${message.guild.members.cache.filter(m => m.user.presence.status === "streaming").size} Yayında \n${client.emojis.cache.get(client.emojiler.rahatsızetme)} ${message.guild.members.cache.filter(m => m.user.presence.status === "dnd").size} Rahatsız Etmeyin \n${client.emojis.cache.get(client.emojiler.çevrimdışı)} ${message.guild.members.cache.filter(m => m.user.presence.status === "offline").size} Görünmez \n${client.emojis.cache.get(client.emojiler.bot)} ${message.guild.members.cache.filter(m => m.user.bot).size} Bot`)
