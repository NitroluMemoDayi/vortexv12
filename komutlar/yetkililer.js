...
for(var i = 0; i < message.guild.members.size; i++) {
   if(message.guild.members.array()[i].hasPermission("MANAGE_MESSAGES") && message.guild.members.array()[i].presence.status === "dnd" && !message.guild.members.array()[i].user.bot) {
      str += `${client.emojis.cache.get(client.emojiler.rahatsızetme)} ${message.guild.members.array()[i].user.tag}\n`
    }
      ...
      str += `${client.emojis.cache.get(client.emojiler.çevrimiçi)} ${message.guild.members.array()[i].user.tag}\n`
    }
      ...
      str += `${client.emojis.cache.get(client.emojiler.boşta)} ${message.guild.members.array()[i].user.tag}\n`
    }
      ...
      str += `${client.emojis.cache.get(client.emojiler.çevrimdışı)} ${message.guild.members.array()[i].user.tag}\n`
    }
}
