...
exports.run = async (client, msg, args) => {
  ...
  if (opponent.bot) return msg.reply('Botlar ile oynayamazsın!');
		...
		try {
			...
			const cache.filter = res => [opponent.id, msg.author.id].includes(res.author.id) && res.content.toLowerCase() === word;
			const winner = await msg.channel.awaitMessages(cache.filter, {
				max: 1,
				time: 30000
			});
			this.fighting.delete(msg.channel.id);
