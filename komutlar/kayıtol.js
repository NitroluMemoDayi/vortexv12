...
const cache.filter = m => m.author.id === message.author.id;
...
.then(async () => {
   ...
   if (collected.first().content === sifre) { 
     ...
     message.member.roles.add(db.fetch(`kayıtR_${message.guild.id}`)).then(d => {   message.channel.overwritePermissions(every, {
  SEND_MESSAGES: false
