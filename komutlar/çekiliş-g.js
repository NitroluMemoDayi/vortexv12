...
var cache.filter = m => m.author.id === message.author.id;
      ...
      message.channel.awaitMessages(cache.filter, {
        max: 1,
        time: 20000,
        errors: ['time']
      }).then(collected => {
                ...
                try {
                  ...
                  message.guild.channels.cache.find("name" , room).send(' :heavy_check_mark: **ÇEKİLİŞ BAŞLADI** :heavy_check_mark:' , {embed: giveEmbed}).then(m => {
                     ...
                     setTimeout(() => {
                       ...
                       let list = users.array().cache.filter(u => u.id !== m.author.id !== client.user.id);
                       let gcache.filter = list[Math.floor(Math.random() * list.length) + 0]
                       let endEmbed = new Discord.MessageEmbed()
                       .setAuthor(message.author.username, message.author.avatarURL())
                       .setTitle(title)
                       .setColor("#f558c9")
            .setFooter("(Vortex çekiliş sistemi)")
                       .addField('Çekiliş Bitti !🎉',`Kazanan : ${gcache.filter} \nBitiş zamanı :`)
                       .setTimestamp()
                    ...
                    message.guild.channels.cache.find("name" , room).send(`**Tebrikler ${gcache.filter}! \`${title}\` kazandın!**` , embedLel)
                }, ms(duration));
            });
                } catch(e) {
