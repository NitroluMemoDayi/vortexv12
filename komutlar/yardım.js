...
${footer}`];
            ...
            const müzikE = new Discord.MessageEmbed()
            .setColor('RANDOM')
 ...
 // let yana = await bot.emojis.cache.get(bot.emojiler.yan);
let yana = "-";
...
arr.push(x.conf.kategori)
  ...
  let cats = arr.cache.filter(x => x !== undefined).map(k => `• \`${prefix}${dil.yardımm} ${k} \`-> ${k.charAt(0).toUpperCase()+k.slice(1)} ${dil.showw}`).join("\n")
...
.setTimestamp()
  ...
  let list = client.commands.cache.filter(x => x.conf.kategori === arg)
