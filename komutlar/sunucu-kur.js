...
\`\`\`
    	...
    	message.guild.channels.create('sayaç', 'text').then(sayac => {
...
\`\`\`
   	...
   	message.guild.channels.create('resimli-hoşgeldin', 'text').then(gcc => {
...
\`\`\`
    	...
    	db.set(`log_${message.guild.id}`, log.id)
      	...
      	message.guild.channels.create('güvenlik', 'text').then(guvenlik => {
