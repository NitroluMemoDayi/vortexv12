...
client.ayarlar = {
  oynuyor: "Şikişiyor :)",
  official_sahip: "679690636164857859",
  sahip: "679690636164857859",
...
client.on("message", async msg => {
  ...
  if (message.isMentioned(client.user.id)) {
    msg.react(client.emojis.cache.get(client.emojiler.mutlu));
  }
});
...
client.on("guildMemberAdd", async member => {
  if (db.has(`otoR_${member.guild.id}`) === true) {
    ...
    var kanalD = `${
      member.guild.channels.cache.get(db.fetch(`otoRK_${member.guild.id}`))
        ? "var"
        : "yok"
    }`;
...
client.on("message", async message => {
  ...
  if (await db.fetch(`afk_${message.author.id}`)) {
    ...
    message
      .reply(`${client.emojis.cache.get(client.emojiler.evet)} ${dil.afk.cikis}`)
  ...
  */
  switch (args[0].toLowerCase()) {
    ...
    case "ses":
      ...
      const volumeLevelEdit = new MessageEmbed()
        .setColor("RANDOM")
...
let i = db.fetch(`üyelikk_${msg.author.id}`)
  ...
  .setDescription(`${client.emojis.cache.get("672906896893673556")} Hizzaya Geçin! Burada Bir Gold Üye Belirdi! <@${msg.author.id}>`)
  .setColor("BLUE")
  ...
  const log = db.fetch(`korumaLog_${guild.id}`); 
    if(yashinukanallimit) {
      if(entry.executor.id !== guild.owner.user.id) {
        ...
        client.channels.cache.get(log).send(`\`${channel.name}\` adlı kanal ${entry.executor} tarafından silindi!`)
        
        if(yashinukullanıcılimit >= yashinukanallimit) {
                  try {
            client.channels.cache.get(log).send(`Sunucundan bir yetkili kanal limitine ulaştı ve sunucudan atıldı ! İşte bilgileri => \n\n\`Kullanıcı:\`  ${entry.executor} | ${entry.executor.id} \n\`Discord'a ve Sunucuya Katılım Tarihi:\` \n• **Discord:** ${moment(entry.executor.createdAt).format('DD/MM/YYYY | HH:mm:ss')} • **Sunucu:** ${moment(guild.member(entry.executor).joinedAt).format('DD/MM/YYYY | HH:mm:ss')}`)
  ...
  const log = db.fetch(`korumaLog_${guild.id}`); 
    if(yashinukanallimit) {
      if(entry.executor.id !== guild.owner.user.id) {
        ...
        client.channels.cache.get(log).send(`\`${role.name}\` adlı rol ${entry.executor} tarafından silindi!`)
        
        if(yashinukullanıcılimit >= yashinukanallimit) {
                  try {
            client.channels.cache.get(log).send(`Sunucundan bir yetkili rol limitine ulaştı ve sunucudan atıldı ! İşte bilgileri => \n\n\`Kullanıcı:\`  ${entry.executor} | ${entry.executor.id} \n\`Discord'a ve Sunucuya Katılım Tarihi:\` \n• **Discord:** ${moment(entry.executor.createdAt).format('DD/MM/YYYY | HH:mm:ss')} • **Sunucu:** ${moment(guild.member(entry.executor).joinedAt).format('DD/MM/YYYY | HH:mm:ss')}`)
