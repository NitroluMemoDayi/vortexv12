...
member.setNickname(`${tag} ${member.user.username}`)
  ...
  if (db.has(`sayac_${member.guild.id}`) === true) {
    ...
    member.guild.channels.cache.get(channel).send(`<a:acik:690266274336342068> <a:giris:617377064898527241> **${member.user.tag}** Sunucuya katıldı! \`${db.fetch(`sayac_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.members.size}\` üye kaldı!`)
    }};
